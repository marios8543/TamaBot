...
class Result():
	def dictate(self):
		...
		return dicc
"""
async def sqlite_connect(database=None,loop=None):
	...
	return ORM(db,conn)
"""
async def mysql_connect(host=None,user=None,pw=None,db=None,loop=None):
	conn = await aiomysql.connect(host=host,user=user,password=pw,db=db,loop=loop)
	db = await conn.cursor()
	for q in queries:
		await db.execute(q)
	return [conn,db]
